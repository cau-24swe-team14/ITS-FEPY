...
def loginView(view):
    ...
    button_login = tk.Button(frame_buttons, text="로그인", font=("Arial", 14), command=lambda: login(view, entry_username, entry_password))
    button_login.grid(row=0, column=0, padx=10)

    button_register = tk.Button(frame_buttons, text="회원가입", font=("Arial", 14), command=lambda: register(view))
